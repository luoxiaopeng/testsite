...
class SortedFilteredSelectMultiple(forms.SelectMultiple):
    ...
    def render_options(self, choices, selected_choices):
        ...
        if hasattr(queryset.model, 'source') and queryset.model.__name__ != 'Workout':
            fields_name = ['id', 'source__name']
        elif queryset.model.__name__ == 'Music':
            fields_name = ['id', 'display_name']
        else:
            fields_name = ['id', 'name']

        for value in queryset.values_list(*fields_name):
            option_value = value[0]
            option_label = value[1]
